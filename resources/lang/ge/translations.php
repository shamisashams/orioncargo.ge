<?php
return [
    "home" => "მთავარი",
    "about" => "ჩვენს შესახებ",
    "services" => "სერვისები",
    "projects" => "პროექტები",
    "contact" => "კონტაქტი",
    "hero_title" => "სწრაფი და საიმედო საერთაშორისო გადაზიდვის სერვისი",
    "hero_content" => "საჰაერო გადაზიდვების სრული სპექტრი პარტნიორი ავიაკომპანიების მეშვეობით.",
    "watch_the" => "ნახეთ ვიდეო",
    "video" => "video",
    "social_links" => "სოციალური ქსელები",
    "working_hours" => "სამუშაო საათები",
    "days" => "ორშაბათი-კვირა",
    "hours" => "09:00 - 19:00",
    "address" => "მისამართი",
    "street_name" => "თბილისის ",
    "country" => "საერთაშორისო აეროპორტი",
    "phone" => "ტელეფონი",
    "number_1" => "+995 0322 14 15 16",
    "number_2" => "+995 0322 14 15 16",
    "from" => "საიდან",
    "to" => "სად",
    "date" => "თარიღი",
    "your_email" => "თქვენი ელ-ფოსტა",
    "get_a_quote" => "გაგზავნა",
    "on_time_deliver" => "ჩვენ ვერგებით თქვენს საჭიროებებს",
    "send_request" => "გაგზავნა",
    "air_freight" => "საერთაშორისო საჰაერო გადაზიდვები",
    "air_freight_desc" => "კომპანია 'ორიონ კარგო' გთავაზობთ საჰაერო გადაზიდვების სრულ სპექტრს, პარტნიორი  ავიაკომპანიების მეშვეობით.
    •	საერთაშორისო საჰაერო გადაზიდვები: ექსპორტი, იმპორტი, ტრანზიტი
•	კონკურენტული ტარიფები
•	ტვირთის დაზღვევა (კლინეტის მოთხოვნის შესაბამისად)
•	ადგილობრივი მომსახურებების უზრუნველყოფა საქართველოს მასშტაბით
•	კარიდან-კარამდე მიტანის მომსახურება
•	საბაჟო-საბროკერო მომსახურება
•	ტვირთის დასაწყობება ტერმინალში.
•	საერთაშორისო დოკუმენტაციის დამზადება
•	ხარისხიანი და სანდო მომსახურების უზრუნველყოფა
",
    "land_freight" => "სახმელეთო გადაზიდვები",
    "land_freight_desc" => "კომპანია 'ორიონ კარგო' გთავაზობთ სახმელეთო გადაზიდვების სრულ სპექტრს:
    •	საერთაშორისო სახმელეთო გადაზიდვები: ექსპორტი, იმპორტი, ტრანზიტი
•	მთლიანი ავტოსატრანსპორტო საშუალება (FTL)
•	ნაკრები ტვირთების გადაზიდვა (LTL)
•	საკონტეინერო გადაზიდვები
•	კონკურენტული ტარიფები
•	ტვირთის დაზღვევა (კლინეტის მოთხოვნის შესაბამისად)
•	ადგილობრივი მომსახურებების უზრუნველყოფა საქართველოს მასშტაბით
•	კარიდან-კარამდე მიტანის მომსახურება
•	საბაჟო-საბროკერო მომსახურება
•	ტვირთის დასაწყობება ტერმინალში.
•	საერთაშორისო დოკუმენტაციის დამზადება
•	ტვირთის გადაზიდვისთვის სპეციალური ნებართვების დამზადება
•	ხარისხიანი და სანდო მომსახურების უზრუნველყოფა",
    "cargo_shipping" => "Cargo shipping",
    "cargo_shipping_desc" => "Leverage agile frameworks to provide a robust synopsis for high level overviews. Iterative approaches to strategy.",
    "refrigerated_cargo" => "ტემპერატურული რეჟიმი",
    "refrigerated_cargo_desc" => "კომპანია ორიონ კარგო გთავაზობთ ტემპერატურული რეჟიმებით უზრუნველყოფას სპეციალური საჭიროების ტვირთისთვის.
    •	ტემპერატურული რეჟიმის უზრუნველყოფა ავია გადაზიდვისას
    •	ტემპერატურული რეჟიმის უზრუნველყოფა სახმელეთო გადაზიდვისას
    •	ტემპერატურული რეჟიმის უზრუნველყოფა როგორც გაგზავნის ასევე სატრანზიტო ტერმინალებში
    •	მშრალი ყინულით უზრუნველყოფა
    ",
    "charter_flights" => "ჩარტერული რეისები",
    "charter_flights_desc" => "კომპანია ორიონ კარგო გთავაზობთ ჩარტერული რეისებით მომსახურებას. 
    •	ჩარტერული ავია რეისების ორგანიზება ნებისმიერი მიმართულებით
    •	თვითმფრინავის ტიპის შერჩევა ტვირთის წონის და გაბარიტებიდან გამომდინარე
    •	ჩარტერული ავია რეისების სახიფათო ტვირთის გადაზიდვისათვის
    •	VIP რეისების ორგანიზება
    •	კონკურენტული ტარიფები
    •	ტვირთის დაზღვევა (კლინეტის მოთხოვნის შესაბამისად)
    •	ადგილობრივი მომსახურებების უზრუნველყოფა საქართველოს მასშტაბით
    •	კარიდან-კარამდე მიტანის მომსახურება
    •	საბაჟო-საბროკერო მომსახურება
    •	ტვირთის დასაწყობება ტერმინალში.
    •	საერთაშორისო დოკუმენტაციის დამზადება
    •	ტვირთის გადაზიდვისთვის სპეციალური ნებართვების დამზადება
    •	ხარისხიანი და სანდო მომსახურების უზრუნველყოფა
    ",
    "good_shipping" => "სახიფათო ტვირთების გადაზიდვა",
    "good_shipping_desc" => "სახიფათო ტვირთების გადაზიდვა ყველა საერთაშორისო სტანდარტის მიხედვით.",
    "customs_brokerage_services" => "საბაჟო-საბროკერო მომსახურება",
    "customs_brokerage_services_desc" => "კომპანია „კარგო“ გთავაზობთ საბაჟო საბროკერო მომსახურების სრულ სპექტრს.
    •	სატრანზირო ფორმალობების განხორციელება
    •	საექსპორტო ფორმალობების განხორციელება
    •	საიმპორტო ფორმალობების განხორციელება
    •	დოკუმენტაციის მომზადება
    •	საკონსულტაციო მომსახურება
    •	ტვირთის ჩამოსვლამდე, საბუთების წინასწარ გადამოწმება
    ",
    "our_Services" => "სერვისები",
    "need" => "ჩვენ ვერგებით თქვენს საჭიროებებს",
    // "solution" => "We Have The SOLUTION",
    "gallery" => "გალერეა",
    "our_projects" => "Our Projects",
    "see_our_projects" => "See Our Projects",
    "see_our_projects_desc" => "Since its inception, the company has been striving to attract and
              sign up for long-term partnerships with customers in the
              marketplace with whom we share common values, both personally and
              in business, not just in business, but in building friendly
              relationships with existing and future partners.",
    "who_are_we" => "ვინ ვართ ჩვენ",
    "who_are_we_desc" => "
    კომპანია ორიონ კარგო საჰაერო და სახმელეთო გადაზიდვების სფეროში 2020 წლიდან ოპერირებს და მომხმარებელს სთავაზობს მის მოთხოვნებზე მორგებული სერვისების სრულ სპექტრს. 
              ",
    "frequently_asked" => "ხშირად დასმული შეკითხვები",
    "question" => "Questions",
    "question_1" => "რომელი ავირჩიო: საჰაერო თუ სახმელეთო გადაზიდვა?",
    "answer_1" => "ეს დამოკიდებულია იმაზე, თუ რამდენად სწრაფად გსურთ მიიღოთ თქვენი ტვირთი. კომპანია ორიონ კარგო ნებისმიერ შემთხვევაში შემოგთავაზებთ გადაზიდვის ოპტიმალურ დროს. ",
    "question_2" => "აქვს თუ არა მნიშვნელობა ტვირთის წონას საჰაერო გადაზიდვის შემთხვევაში?",
    "answer_2" => "ტვირთის წონას დიდი მნიშვნელობა აქვს. გადაზიდვის ღირებულება განისაზღვრება ბრუტო წონისა და გაბარიტული წონის მიხედვით. ",
    "question_3" => "როგორ დგინდება გადაზიდვის ტარიფი?",

    "answer_3_1" => "დაგვიკავშირდით და მიაწოდეთ შემდეგი სახის ინფორმაცია ჩვენს ოპერატორს:",
    "answer_3_2" => "•	წარმოშობის ქალაქი, ქვეყანა და ინდექსი;",
    "answer_3_3" => "•	დანიშნულების ადგილი, ქვეყანა და ინდექსი;",
    "answer_3_4" => "•	ტვირთის სახეობა;",
    "answer_3_5" => "•	ადგილების რაოდენობა;",
    "answer_3_6" => "•	მთლიანი წონა შეფუთვის ჩათვლით;",
    "answer_3_7" => "•	ზომები შეფუთვების ჩათვლით (სიგრძე, სიგანე, სიმაღლე).",
    "answer_3_8" => "ჩვენი თანამშრომელი ამ მონაცემების გათვალისწინებით დაგითვლით ტარიფს და მოგაწვდით ზუსტ ინფორმაციას.",

    "question_4" => "რა სახის დოკუმენტაცია მჭირდება ტვირთის გასაფორმებლად?",
    "answer_4_1" => "ტვირთის გასაფორმებლად გჭირდებათ შემდეგი სახის დოკუმენტაცია: ",
    "answer_4_2" => "•	ზედნადები;",
    "answer_4_3" => "•	კომერციული ინვოისი და ",
    "answer_4_4" => "•	ტვირთის ჩამონათვალი.",
    "answer_4" => " ",
    "question_5" => "სად მდებარეობს თქვენი კომპანიის ოფისი?",
    "answer_5" => "ჩვენი ოფისი განთავსებულია თბილისის საერთაშორისო აეროპორტში. ",
    "more_questions" => "გაქვთ კითხვები?",
    "get_in_touch" => "დაგვიკავშირდით:",
    "name_surname" => "თქვენი სახელი და გვარი",
    "phone_number" => "ტელეფონის ნომერი",
    "email_address" => "ელ-ფოსტა",
    "your_message_here" => "შეტყობინება...",
    "why_choose_us" => "რატომ ჩვენ?",
    "package_deliver" => "ჩვენ უზრუნველვყოფთ თქვენი ტვირთის დროულ გადაზიდვას დათქმულ ვადებში.",
    "benefit_list_title_1" => "დრო",
    "benefit_list_description_1" => "ჩვენ უზრუნველვყოფთ თქვენი ტვირთის დროულ გადაზიდვას დათქმულ ვადებში.",
    "benefit_list_title_2" => "ღირებულება",
    "benefit_list_description_2" => "გთავაზობთ კონკურენტულ ტარიფებს ნებისმიერი ტიპის სატრანსპორტო გადაზიდვისას. ",
    "benefit_list_title_3" => "საერთაშორისო პარტნიორული ქსელი",
    "benefit_list_description_3" => "კომპანია ორიონ კარგოს ჰყავს პარტნიორები საერთაშორისო მასშტაბით, რაც უზრუნველყოფს ჩვენი მომხმარებლებისთვის ტვირთების გადაზიდვის სერვისის სრული და კომფორტული პაკეტის შეთავაზებას.",
    "benefit_list_title_4" => "საიმედოობა",
    "benefit_list_description_4" => "ჩვენი კომპეტენციიდან და საერთაშორისო პარტნიორული ქსელისგან გამომდინარე უზრუნველვყოფთ თქვენი ტვირთის საიმედო გადაზიდვას მსოფლიოს ნებისმიერი წერტილიდან.",
    "about_us" => "ჩვენ შესახებ",
    "about_us_title_1" => "ისტორია",
    "about_us_content_1" => " კომპანია ორიონ კარგო ქართულ ბაზარზე 2020 წლიდან მოღვაწეობს. ამ ხნის განმავლობაში ჩვენ შევძელით გამოცდილების, კომპეტენციის, სანდო პარტნიორების ქსელისა და კმაყოფილი კლიენტების უწყვეტი ნაკადის ბაზის შექმნა.",
    "about_us_title_2" => "საქმიანობა",
    "about_us_content_2" => "ჩვენს მომხმარებელს ვთავაზობთ სწრაფ, ხარისხიან და ინდივიდუალურად მორგებულ სერვისს თითოეულ გადაზიდვაზე. ტვირთების საერთაშორისო გადაზიდვისას ვიცავთ ყველა საერთაშორისო სტანდარტს და მომხმარებელს ვთავაზობთ გამართულ სერვისს.",
    "about_us_title_3" => "ჩვენი უპირატესობა",
    "about_us_content_3" => "ჩვენი კომპანია გამოირჩევა ავიაკომპანიებთან და სახმელეთო გადამზიდავებთან უშუალო პარტნიორობით, რაც განაპირობებს ტვირთების სწრაფ, უსაფრთხო და მომგებიანი პირობებით ტრანსპორტირებას.",
    "contact_us" => "დაგვიკავშირდით",
    "services_desc" => "ჩვენს მომხმარებლებს ვთავაზობთ სერვისების ფართო სპექტრს",
    "email_address_contact" => "ელ-ფოსტა",
    "email_1" => "example@info.ge",
    "email_2" => "example@contact.ge",
    "any_questions" => "თუ გაქვთ დამატებითი შეკითხვები, დაგვიკავშირდით",
    "find_on_map" => "ადგილმდებარეობა",
    "send_message" => "გაგზავნა",
    "project_1" => "project sdf name",
    "project_slug_1" => "project_1",
    "project_para_1" => "Leverage agile frameworks to provide a robust synopsis for high level overviews. Iterative approaches to strategy.",
    "project_2" => "project name akljsdin",
    "project_slug_2" => "project_2",
    "project_para_2" => "Leverage agile frameworks to provide a robust synopsis for high level overviews. Iterative approaches to strategy.",
    "project_3" => "project sdf name",
    "project_slug_3" => "project_3",
    "project_para_3" => "Leverage agile frameworks to provide a robust synopsis for high level overviews. Iterative approaches to strategy.",
    "project_4" => "project sdf name",
    "project_slug_4" => "project_4",
    "project_para_4" => "Leverage agile frameworks to provide a robust synopsis for high level overviews. Iterative approaches to strategy.",
    "we_offer" => "ჩვენს მომხმარებლებს ვთავაზობთ სერვისების ფართო სპექტრს",



    "int_head_1" => "",

    "more_details" => "ნახვა",
    "your_email" => "ელ-ფოსტა",
    "your_phone" => "ტელეფონი",
    "" => "",
    "" => "",


    "airfreight_bullet_1" => "საერთაშორისო საჰაერო გადაზიდვები: ექსპორტი, იმპორტი, ტრანზიტი;",
    "airfreight_bullet_2" => "კონკურენტული ტარიფები;",
    "airfreight_bullet_3" => "ტვირთის დაზღვევა (კლინეტის მოთხოვნის შესაბამისად);",
    "airfreight_bullet_4" => "ადგილობრივი მომსახურებების უზრუნველყოფა საქართველოს მასშტაბით;",
    "airfreight_bullet_5" => "კარიდან-კარამდე მიტანის მომსახურება;",
    "airfreight_bullet_6" => "საბაჟო-საბროკერო მომსახურება;",
    "airfreight_bullet_7" => "ტვირთის დასაწყობება ტერმინალში;",
    "airfreight_bullet_8" => "საერთაშორისო დოკუმენტაციის დამზადება;",
    "airfreight_bullet_9" => "ხარისხიანი და სანდო მომსახურების უზრუნველყოფა.",
    "airfreight_para_1" => "კომპანია ორიონ კარგო გთავაზობთ საჰაერო გადაზიდვების სრულ სპექტრს, პარტნიორი  ავიაკომპანიების მეშვეობით.",
    "airfreight_para_2" => "",
    "brokerage_bullet_1" => "",
    "brokerage_bullet_2" => "საექსპორტო ფორმალობების განხორციელება;",
    "brokerage_bullet_3" => "საიმპორტო ფორმალობების განხორციელება;",
    "brokerage_bullet_4" => "დოკუმენტაციის მომზადება;",
    "brokerage_bullet_5" => "საკონსულტაციო მომსახურება;",
    "brokerage_bullet_6" => "ტვირთის ჩამოსვლამდე, საბუთების წინასწარ გადამოწმება;",
    "brokerage_bullet_7" => "სატრანზირო ფორმალობების განხორციელება.",
    "brokerage_bullet_8" => "",
    "brokerage_para_1" => "კომპანია ორიონ კარგო გთავაზობთ საბაჟო საბროკერო მომსახურების სრულ სპექტრს.",
    "brokerage_para_2" => "",
    "charter_bullet_1" => "ჩარტერული ავია რეისების ორგანიზება ნებისმიერი მიმართულებით;",
    "charter_bullet_2" => "თვითმფრინავის ტიპის შერჩევა ტვირთის წონის და გაბარიტებიდან გამომდინარე;",
    "charter_bullet_3" => "ჩარტერული ავია რეისების სახიფათო ტვირთის გადაზიდვისათვის;",
    "charter_bullet_4" => "VIP რეისების ორგანიზება;",
    "charter_bullet_5" => "კონკურენტული ტარიფები;",
    "charter_bullet_6" => "ტვირთის დაზღვევა (კლინეტის მოთხოვნის შესაბამისად);",
    "charter_bullet_7" => "ადგილობრივი მომსახურებების უზრუნველყოფა საქართველოს მასშტაბით;",
    "charter_bullet_8" => "კარიდან-კარამდე მიტანის მომსახურება;",
    "charter_bullet_9" => "საბაჟო-საბროკერო მომსახურება;",
    "charter_bullet_10" => "ტვირთის დასაწყობება ტერმინალში;",
    "charter_bullet_11" => "საერთაშორისო დოკუმენტაციის დამზადება;",
    "charter_bullet_12" => "ტვირთის გადაზიდვისთვის სპეციალური ნებართვების დამზადება;",
    "charter_bullet_13" => "ხარისხიანი და სანდო მომსახურების უზრუნველყოფა.",
    "charter_para_1" => "კომპანია ორიონ კარგო გთავაზობთ ჩარტერული რეისებით მომსახურებას. ",
    "charter_para_2" => "",
    "danger_bullet_1" => "ტვირთის ანალიზი და კლასიფიკაციის დადგენა;",
    "danger_bullet_2" => "საშიში ტვირთის დეკლარაციის დამზადება;",
    "danger_bullet_3" => "მარკირება და შესაბამისი პირობებით შენახვა;",
    "danger_bullet_4" => "ესკორტით გაცილება;",
    "danger_bullet_5" => "მიწოდება კარიდან–კარამდე.",
    "shipping_para_1" => "სახიფათო ტვირთების გადაზიდვა ყველა საერთაშორისო სტანდარტის მიხედვით. ",
    "shipping_para_2" => "",
    "landfreight_bullet_1" => "საერთაშორისო სახმელეთო გადაზიდვები: ექსპორტი, იმპორტი, ტრანზიტი;",
    "landfreight_bullet_2" => "მთლიანი ავტოსატრანსპორტო საშუალება (FTL);",
    "landfreight_bullet_3" => "ნაკრები ტვირთების გადაზიდვა (LTL);",
    "landfreight_bullet_4" => "საკონტეინერო გადაზიდვები;",
    "landfreight_bullet_5" => "კონკურენტული ტარიფები;",
    "landfreight_bullet_6" => "ტვირთის დაზღვევა (კლინეტის მოთხოვნის შესაბამისად);",
    "landfreight_bullet_7" => "ადგილობრივი მომსახურებების უზრუნველყოფა საქართველოს მასშტაბით;",
    "landfreight_bullet_8" => "კარიდან-კარამდე მიტანის მომსახურება;",
    "landfreight_bullet_9" => "საბაჟო-საბროკერო მომსახურება;",
    "landfreight_bullet_10" => "ტვირთის დასაწყობება ტერმინალში;",
    "landfreight_bullet_11" => "საერთაშორისო დოკუმენტაციის დამზადება;",
    "landfreight_bullet_12" => "ტვირთის გადაზიდვისთვის სპეციალური ნებართვების დამზადება;",
    "landfreight_bullet_13" => "ხარისხიანი და სანდო მომსახურების უზრუნველყოფა.",
    "landfreight_para_1" => "კომპანია ორიონ კარგო გთავაზობთ სახმელეთო გადაზიდვების სრულ სპექტრს:",
    "landfreight_para_2" => "",
    "refrigerated_bullet_1" => "ტემპერატურული რეჟიმის უზრუნველყოფა ავია გადაზიდვისას;",
    "refrigerated_bullet_2" => "ტემპერატურული რეჟიმის უზრუნველყოფა სახმელეთო გადაზიდვისას;",
    "refrigerated_bullet_3" => "ტემპერატურული რეჟიმის უზრუნველყოფა როგორც გაგზავნის ასევე სატრანზიტო ტერმინალებში;",
    "refrigerated_bullet_4" => "მშრალი ყინულით უზრუნველყოფა.",
    "refrigerated_bullet_5" => "",
    "refrigerated_bullet_6" => "",
    "refrigerated_bullet_7" => "",
    "refrigerated_bullet_8" => "",
    "refrigerated_para_1" => "კომპანია ორიონ კარგო გთავაზობთ ტემპერატურული რეჟიმებით უზრუნველყოფას სპეციალური საჭიროების ტვირთისთვის.",
    "refrigerated_para_2" => "",


    "see_more_services" => "ჩვენი სერვისები"

];?>


];?>
